...
def main():
    ...
    text_prop = textprop.TexturePropagation(
        struct_prop.img_output, struct_prop.updated_structure_mask, struct_prop.unknown_mask, struct_prop.patch_size
    )
    text_prop.run()
    text_prop.plot_img(
        text_prop.img_output,
        os.path.join(os.path.abspath(os.path.curdir), "{}-inpaint.png".format(args.image_source)),
    )
    if text_prop.img_output_2 is not None:
        text_prop.plot_img(
            text_prop.img_output_2,
            os.path.join(os.path.abspath(os.path.curdir), "{}-txfill-down.png".format(args.image_source)),
        )
    if text_prop.img_output_3 is not None:
        text_prop.plot_img(
            text_prop.img_output_3,
            os.path.join(os.path.abspath(os.path.curdir), "{}-txfill-up.png".format(args.image_source)),
        )
    if text_prop.img_output_4 is not None:
        text_prop.plot_img(
            text_prop.img_output_4,
            os.path.join(os.path.abspath(os.path.curdir), "{}-txfill-col-left.png".format(args.image_source)),
        )
    if text_prop.img_output_5 is not None:
        text_prop.plot_img(
            text_prop.img_output_5,
            os.path.join(os.path.abspath(os.path.curdir), "{}-txfill-col-right.png".format(args.image_source)),
        )
